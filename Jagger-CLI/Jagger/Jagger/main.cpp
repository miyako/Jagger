...
//  Jagger
//
//  Created by miyako on 2025/05/01.
//
...
int main(int argc, char** argv) {
    ...
    std::string defaultdict = parentdir;
    defaultdict += ("dict" SEP DEFAULT_DICNAME SEP "dict");

    std::string m = parentdir;
    m += ("model" SEP DEFAULT_MODELNAME SEP "patterns");

    bool tagging  = true;
    
    std::string h = std::string("Pattern-based Jappanese Morphological Analyzer\n\
Copyright (c) 2023- Naoki Yoshinaga, All rights reserved.\n\nUsage: ");
    h += argv[0];
    h += " [-m dir w]\n\nOptions:\n -m dir\tdirectory for compiled patterns (default: ";
    h += m;
    ...
    while (running) {
        ...
        pipe(stdin_pipe);
    #else
        _pipe(stdin_pipe, line.size() + 1, _O_BINARY);
    #endif
